    ...
    for review_text, highlights in review_to_highlight_dict.items():
        ...
        for sent, highlight_detail, highlight_reason in highlights:
            if sent == '+':
                ...
                sent = '?'
            if display_reason and highlight_reason:
                highlight_htmls.append(f'<span style="background-color: {color_mapping[sent]}; border: 5px solid {color_mapping[detail_axis]}; color: black; padding: 5px;">{highlight_detail}</span> <span style="background-color: {color_mapping[sent]}; border: 5px solid {color_mapping["y"]}; color: black; padding: 5px;">{highlight_reason}</span>')
            else:
                highlight_htmls.append(f'<span style="background-color: {color_mapping[sent]}; border: 5px solid {color_mapping[detail_axis]}; color: black; padding: 5px;">{highlight_detail}</span>')
...
if __name__ == '__main__':
    update_graph('use_attr_perf', 'all', 'all', 10, 10, 'en', 0, '')